...
float DoInterp(unsigned i, unsigned N, const char *Name,
  bool Set, bool SetStart, bool SetEnd,
  double Val, double ValStart, double ValEnd)
	{
	asserta(i < N);
	asserta(N > 0);
	double f = (i == 0 ? 0.0f : float(i)/(N - 1));
	asserta(f >= 0.0f && f <= 1.0);
	double v = DBL_MAX;
	if (SetStart)
		{
		asserta(SetEnd);
		v = (1.0f - f)*ValStart + f*ValEnd;
		}
	else
		v = Val;
	return float(v);
...
void myassertfail(const char *Exp, const char *File, unsigned Line)
	{
	Die("%s(%u) asserta failed: %s", File, Line, Exp);
	}
...
#endif
	;
	fprintf(f, "tdg v%s.%s_%s%s", MY_VERSION, SVN_VERSION, GetPlatform(), Flags);
	}
...
void PrintCopyright(FILE *f)
	...
	fprintf(f, "(C) Copyright 2018 Robert C. Edgar, all rights reserved.\n");
	}
