...
#define asserteq(x, y)	asserta(feq(x, y))
...
const float PLUS_INF = 9e9f;
const float MINUS_INF = -9e9f;
...
#define Interp(i, N, x) \
	DoInterp(i, N, #x, \
	  optset_##x, optset_##x##_start, optset_##x##_end, \
	  opt_##x, opt_##x##_start, opt_##x##_end);

#define zerof(a)	std::fill(a.begin(), a.end(), 0.0f)
